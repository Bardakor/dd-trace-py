...
1. **Test Suite Execution**:
   * Running a uv test environment for an integration automatically updates its version information in the registry when needed
...
2. **Manual Update Script**:
  ...
  ***Registry Update Example (Incorrect Workflow)***:
  ...
  - Manually run `python scripts/integration_registry/update_and_format_registry.py` without running the test environment for `integration_a`.
  ...
  ***Registry Update Example (Correct Workflow)***:
  - Add support for new `integration_a`, including patch files and tests
  - Run the full uv test environment for the new `integration_a` suite. This is needed because we cannot reliably map dependency names to integration names when they differ, as with `rediscluster` and `redis-py-cluster`. During the test run, [`IntegrationRegistryManager`](../../../tests/contrib/integration_registry/registry_update_helpers/integration_registry_manager.py) records the patched module and resolves its distribution through `importlib.metadata`.
...
2. Add tests and a corresponding named environment under [`tests/environments`](../../../tests/environments/)
...
* [`test_environments.py`](../../../tests/contrib/integration_registry/test_environments.py):
  * Verifies every integration has corresponding test environments in the uv inventory:
    * Checks that each integration directory has a matching named environment
    * Excludes explicitly untested integrations
    * Reports missing test environment definitions
  * Validates test paths in uv environments:
...
3. Execute the test suite and place a breakpoint in the `IntegrationRegistryUpdater`. The orchestrator obtains `filelock` and `pyyaml` from uv's cached tool environment, so they do not belong in the integration's dependency profile.
...
* [`IntegrationUpdateOrchestrator`](../../../tests/contrib/integration_registry/registry_update_helpers/integration_update_orchestrator.py)
  - Runs the updater in a cached uv tool environment so test environments do not create nested virtual environments.
