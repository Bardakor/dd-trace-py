...
1. Set up an environment, for example with `pip install ddtrace` or the uv test runner.
1. Run the script:
...
1. List the matching uv environment IDs with `python scripts/test_environments.py list ci_visibility`.
1. Make sure the `testagent` is running (refer to contributor docs again)
1. Run the test through the pinned environment.
   1. All tests: `scripts/ddtest scripts/run-uv-test-env <environment_id> -- -k FakeApiRunnersSnapshotTestCase`
   1. Individual test: `scripts/ddtest scripts/run-uv-test-env <environment_id> -- -k test_manual_api_fake_runner_mix_fail_itr_test_level`
