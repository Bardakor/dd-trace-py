...
@pytest.fixture(scope="module")
def test_environment_instances() -> Any:
    """Return the resolved test environment instances."""
    return test_environments.environments(environ={})


@pytest.fixture(scope="module")
def test_environment_names(test_environment_instances: Any) -> set[str]:
    """Return every named test environment node."""
    names = {environment.name for environment in test_environment_instances}
    if not names:
        pytest.fail("No named test environments found in the inventory.")
    return names
