...
def collect_global_attributes(record_testsuite_property, pytestconfig):
    ...
    # Record the resolved environment so CI failures can be reproduced locally.
    for env, value in os.environ.items():
        if not env.startswith("DD_TEST_ENV_"):
            continue
        attribute = "test.environment." + env.removeprefix("DD_TEST_ENV_").lower().replace("_", ".")
        record_testsuite_property(attribute, value)
